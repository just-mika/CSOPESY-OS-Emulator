...
std::string PrintCommand::processStringToPrint() {
    auto process = GlobalScheduler::getInstance()->findProcess(pid);
    if (!process) return this->toPrint;
    ...
    return finalOutput; // never touches this->toPrint
}

std::string PrintCommand::getToPrint() {
    return processStringToPrint();
}
...
void PrintCommand::execute() {
    ...
    std::string output = processStringToPrint();
    FileLogger::logCommandExecution(process->getName(), process->getCPUCoreID(), output);
}